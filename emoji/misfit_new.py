...
import os, sys
sys.path.insert(0, "/Users/mirjetapasha/Documents/Research_Projects/Besov_jan11/Spatiotemporal-Besov-prior")
...
class misfit(object):
    ...
    def _gen_emoji(self, dataset = 30):
        """
        ...
        if dataset == 10:
            ...
            if not os.path.exists('./data'): os.makedirs('./data')
            if not os.path.exists('./data/DataDynamic_128x60.mat'):
                import requests
                print("downloading...")
                r = requests.get('https://zenodo.org/record/1183532/files/DataDynamic_128x60.mat')
                with open('./data/DataDynamic_128x60.mat', "wb") as file:
                    file.write(r.content)
                print("Emoji data downloaded.")
            with h5py.File('./data/DataDynamic_128x60.mat', 'r') as f:
                A = sps.csc_matrix((f["A"]["data"], f["A"]["ir"], f["A"]["jc"]))
                normA = np.array(f['normA'])
                sinogram = np.array(f['sinogram']).T
            T = 33
            ...
            # Need this
            # for ii in range(T):
            #     AA[ii] = A_small[ 6510*(ii):6510*(ii+1), 16384*ii:16384*(ii+1) ] # 217 projections of size 128x128 at each of 10 selected angles
            #     B[ii] = b[ 6510*(ii) : 6510*(ii+1) ]
            for ii in range(T):
                AA[ii] = A_small[ 2170*(ii):2170*(ii+1), 16384*ii:16384*(ii+1) ] # 217 projections of size 128x128 at each of 10 selected angles
                B[ii] = b[ 2170*(ii) : 2170*(ii+1) ]
    ...
    def observe(self):
        ...
        """
        A, b, AA, B, nx, ny, nt, delta = self._gen_emoji(30)
