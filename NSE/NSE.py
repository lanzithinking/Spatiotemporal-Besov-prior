#!/usr/bin/env python
"""
Class definition of the dynamic non-linear example Navier-Stokes equation (NSE)
Shiwei Lan @ ASU 2022
-------------------------------------------------------------------------------
Created March 7, 2023 for project of Spatiotemporal Besov prior (STBP)
"""
...
class NSE:
    ...
    def _init_param(self,init_opt='truth',**kwargs):
        """
        Initialize parameter with a quick but rough reconstruction
        """
        self.init_parameter = {'truth':self.prior.fun2vec(self.misfit.truth.cpu().numpy()),
                               'prior_sample':self.prior.sample()}[init_opt]
    ...
    def _get_misfit(self, parameter, MF_only=True):
        ...
        msft = self.misfit.cost(self.prior.vec2fun(parameter)).detach().cpu().numpy()
    ...
    def _get_grad(self, parameter, MF_only=True):
        ...
        """
        # obtain the gradient
        grad = self.prior.fun2vec(self.misfit.grad(self.prior.vec2fun(parameter)).cpu().numpy().flatten(order='F'))
    ...
    def _get_HessApply(self, parameter, MF_only=True):
        ...
        """
        # obtain the Hessian
        hess_ = lambda v: self.prior.fun2vec(self.misfit.Hess(self.prior.vec2fun(parameter))(self.prior.vec2fun(v)).cpu().numpy())
...
if __name__ == '__main__':
    ...
    store_eig = True
    nse = NSE(spat_args=spat_args, temp_args=temp_args, store_eig=store_eig, seed=seed)
    # test
    nse.test(1e-7, MF_only=False)
    # obtain MAP
    map_v = nse.get_MAP(SAVE=True, NCG=True)#,init_opt='truth')
    print('MAP estimate: '+(min(len(map_v),10)*"%.4f ") % tuple(map_v[:min(len(map_v),10)]) )
    #  compare it with the truth
    true_param = nse.misfit.truth.cpu().numpy().flatten(order='F')
    map_f = nse.prior.vec2fun(map_v).reshape(true_param.shape)
    ...
    min_cost = nse._get_misfit(map_v)
    print('Minimum cost: %.4f' % min_cost)
    # plot MAP
    map_f = nse.prior.vec2fun(map_v).reshape(np.append(nse.misfit.sz_x,nse.misfit.sz_t),order='F')
    nse.misfit.plot_data(dat_imgs=map_f, save_imgs=True, save_path='./invsol/MAP')