...
def compl_mul3d(input, weights):
    ...
    return torch.view_as_real(torch.einsum("bixyz,ioxyz->boxyz", input, weights))
...
class SpectralConv3d_fast(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.fft.irfftn(torch.view_as_complex(out_ft), s=(x.size(-3), x.size(-2), x.size(-1)))
        return x
...
if __name__ == '__main__':
    ...
    # load model
    model = torch.load('model/ns_fourier_V1e-4_T20_N9800_ep200_m12_w32', map_location=device)
