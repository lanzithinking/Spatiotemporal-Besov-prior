...
def _eigen_randproj(Omega,Y,srt_ord='asc'):
    """
...
def eigen_RA_rank(A,dim=None,which='LM',k=20,p=10):
    """
...
def eigen_RA_prec(A,dim=None,which='LM',threshold=0.01,increment_k=20,p=10):
    """
    ...
    while num_eigs<dim+np.float_(increment_k)/2:
        eigv_k,eigf_vec_k=_eigen_randproj(Omega,Y,srt_ord={'SM':'asc','LM':'dsc'}[which])
...
def _geigen_randproj(Omega,Y_bar,Y,B,srt_ord='asc'):
    """
...
def geigen_RA_rank(A,B,invB,dim=None,which='LM',k=20,p=10):
    """
...
def geigen_RA_prec(A,B,invB,dim=None,which='LM',threshold=0.01,increment_k=20,p=10):
    """
    ...
    BOmega=None
    while num_eigs<dim+np.float_(increment_k)/2:
        eigv_k,eigf_vec_k=_geigen_randproj(Omega,Y_bar,Y,B,srt_ord={'SM':'asc','LM':'dsc'}[which])
        eigv_k=eigv_k[:increment_k]
        eigf_vec_k=eigf_vec_k[:,:increment_k]
        # threshold
        idx = eigv_k>=threshold
