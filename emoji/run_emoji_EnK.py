...
def main(seed=2022):
    ...
    #     G = lambda u: np.stack(Parallel(n_jobs=n_jobs)(delayed(lambda u_j: Au(emj.prior.vec2fun(u_j)))(u_j) for u_j in u))
    # else:
    #     G = lambda u: np.stack([Au(emj.prior.vec2fun(u_j)) for u_j in u])
    # import time
    # t0=time.time()
    # res0=G(u0)
    # t1=time.time()
    ...
    eigv, eigf = emj.prior.bsv.eigs() # (I,L)
    tA = [emj.misfit.obs[0][t].dot(eigf) for t in range(emj.misfit.sz_t)] # each of size (m,L)
    tAu = lambda u: np.stack([tA[t].dot(u_t) for t,u_t in enumerate(u.reshape(emj.misfit.sz_t,-1))]).T
    if args.ensemble_size>200:
        n_jobs = np.min([10, multiprocessing.cpu_count()])
    ...
    emj.misfit.data = np.stack(emj.misfit.obs[1]).T
    ...
    enk = EnK(u0,G,emj.misfit,emj.prior,stp_sz=args.step_sizes[args.algNO],nz_lvl=nz_lvl,err_thld=err_thld,
    ...
    filepath=os.path.join(savepath,'Emoji_'+filename+'.pckl') # change filename
    os.rename(filepath_, filepath)
    f=open(filepath,'ab')
    pickle.dump([emj.misfit.data, emj.prior.cov, args],f)
    f.close()
