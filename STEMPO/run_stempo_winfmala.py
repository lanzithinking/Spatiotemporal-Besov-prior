...
def main(seed=2022):
    ...
    def Lmd(z, q=stpo.prior.qep.q, grad=False):
        _z = z.reshape((-1,stpo.prior.qep.N),order='F') # (L,J)
        nm_z = np.linalg.norm(_z,axis=1)[:,None]
        lmd = stpo.prior.qep.act(_z*nm_z**(2/q-1),alpha=0.5,transp=True)#,chol=False) # (L,J)
        if grad:
            # return lambda v: stpo.prior.qep.act(_z*z.dot(v)*nm_z**(2/q-3)*(2/q-1) + v.reshape((-1,stpo.prior.qep.N),order='F')*nm_z**(2/q-1), alpha=0.5,transp=True)
    ...
    def T(z, q=stpo.prior.bsv.q, grad=False):
        if grad:
            return lambda v,adj=False: Lmd(z, grad=grad)(stpo.prior.C_act(v, 1/q),adj=adj).flatten(order='F') if adj else stpo.prior.C_act(Lmd(z, grad=grad)(v), 1/q)
        else:
            return stpo.prior.C_act(Lmd(z), 1/q)
