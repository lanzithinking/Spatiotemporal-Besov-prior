#!/usr/bin/env python
"""
Class definition of the the example of NOAA temperature.
Shiwei Lan @ ASU 2022
--------------------------------------------------------------------------
...
class NOAATMP:
    ...
    def _init_param(self,init_opt='obs',**kwargs):
        ...
        if init_opt=='prior_sample':
            ...
            if init_opt=='obs':
                self.init_parameter = (self.misfit.obs if self.misfit.missing_idx is None else self.misfit._impute(**kwargs)).flatten(order='F')
            elif init_opt=='LSE':
                self.init_parameter = getattr(self.misfit,'reconstruct_'+init_opt)(**kwargs).flatten(order='F')
...
if __name__ == '__main__':
    ...
    nzlvl = .1
    spat_args={'basis_opt':'Fourier','sigma2':100,'l':.1,'s':1,'q':1.0,'L':2000}
    temp_args={'ker_opt':'matern','sigma2':10,'l':.2,'s':1,'q':1.0,'L':100}
    store_eig = True
    noaatmp = NOAATMP(impute_missing=impute_missing, nzlvl=nzlvl, spat_args=spat_args, temp_args=temp_args, store_eig=store_eig, space='fun', seed=seed)
    # test
    noaatmp.test(1e-6, MF_only=False)
    # obtain MAP
    map_v = noaatmp.get_MAP(SAVE=True, NCG=False,init_opt='LSE',lmda=1)
    print('MAP estimate: '+(min(len(map_v),10)*"%.4f ") % tuple(map_v[:min(len(map_v),10)]) )
    #  compare it with the observation
    obs = noaatmp.misfit.obs # no truth
    map_f = noaatmp.prior.vec2fun(map_v) if noaatmp.prior.space=='vec' else map_v
    map_f = map_f.reshape(obs.shape, order='F')
    relerr = np.linalg.norm(noaatmp.misfit._impute(map_f-obs, method='zero'))/np.linalg.norm(noaatmp.misfit._impute(obs, method='zero'))
    print('Relative error of MAP compared with the observation %.2f%%' % (relerr*100))
    # report the minimum cost
    min_cost = noaatmp._get_misfit(map_v)
    print('Minimum cost: %.4f' % min_cost)
    # plot MAP
    map_f = map_f.reshape(np.append(noaatmp.misfit.sz_x,noaatmp.misfit.sz_t),order='F')
    noaatmp.misfit.plot_reconstruction(rcstr_imgs=map_f, save_imgs=True, save_path='./reconstruction/MAP', snap_idx=np.linspace(0,noaatmp.misfit.sz_t,num=10,endpoint=False).astype(int))