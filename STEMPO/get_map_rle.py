...
if os.path.exists(os.path.join(folder,'map_rle'+('_whiten' if whiten else '')+'.pckl')):
    ...
    for m in range(num_mdls):
        ...
        for f_i in pckl_files:
            try:
                ...
                map=f_read[-3]
                if msft.data_set=='simulation': rle.append(np.linalg.norm(map-truth)/np.linalg.norm(truth))
    ...
    pickle.dump([truth,rle_m,rle_s,loglik_m,loglik_s],f)
    f.close()
if not os.path.exists(os.path.join(folder,'map_summary'+('_whiten' if whiten else '')+'.pckl')):
    maps=np.stack(maps)
    funs=np.stack(funs)
...
# save
import pandas as pd
means = pd.DataFrame(data=np.vstack((rle_m,loglik_m)),columns=mdl_names[:num_mdls],index=['rle','log-lik'])
stds = pd.DataFrame(data=np.vstack((rle_s,loglik_s)),columns=mdl_names[:num_mdls],index=['rle','log-lik'])
