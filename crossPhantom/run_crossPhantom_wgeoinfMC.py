"""
Main function to run whitened (geometric) dimension-independent sampling for the dynamic linear inverse problem of crossPhantom.
----------------------
Shiwei Lan @ ASU, 2022
----------------------
"""
...
def main(seed=2022):
    ...
    parser.add_argument('step_sizes', nargs='?', type=float, default=(1e-7,1e-6,1e-5,1e-3,1e-3))
    ...
    try:
        ...
        #     map=pickle.load(f)
        # f.close()
        # z_init=xph.whiten.stbp2wn(map).flatten(order='F')
        u_init=xph.init_parameter if hasattr(xph,'init_parameter') else xph._init_param(init_opt='LSE',lmda=10)
        z_init=xph.whiten.stbp2wn(u_init).flatten(order='F')
    except Exception as e:
        print(e)
        z_init=np.random.randn({'vec':xph.prior.L*xph.prior.J,'fun':xph.prior.N}[xph.prior.space])
    # h=1e-7; v=np.random.randn(xph.prior.L*xph.prior.J)
    # l,g=xph.get_geom(z_init,geom_ord=[0,1],whiten=True)[:2]; hess=xph.get_geom(z_init,geom_ord=[2],whiten=True)[2]
    # Hv=hess(v)
    # l1,g1=xph.get_geom(z_init+h*v,geom_ord=[0,1],whiten=True)[:2]
    ...
    winfMC=wht_geoinfMC(z_init,xph,args.step_sizes[args.alg_NO],args.step_nums[args.alg_NO],args.algs[args.alg_NO],transformation=xph.whiten.wn2stbp, MF_only=True, whitened=True, k=100)
    ...
    # filename='xph_'+args.algs[args.alg_NO]+'_dim'+str(len(u))+'_'+ctime
    ...
    if xph.prior.space=='vec': samp=xph.prior.vec2fun(samp.T).T
