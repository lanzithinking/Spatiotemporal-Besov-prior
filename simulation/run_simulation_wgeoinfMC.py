...
def main(seed=2022):
    ...
    parser.add_argument('n_x', nargs='?', type=int, default=2**4)
    ...
    data_args={'truth_option':1,'n_x':args.n_x, 'space':'fun'}
    spat_args={'basis_opt':'Fourier','l':.1,'s':1,'q':args.q,'L':2000}
    if args.n_x>=spat_args['L']:                                                
        data_args['space']='vec' 
    ...
    try:
        ...
        #     map=pickle.load(f)
        # f.close()
        # z_init=sml.whiten.stbp2wn(map).flatten(order='F')
        u_init=sml.init_parameter if hasattr(sml,'init_parameter') else sml._init_param(init_opt='LSE',lmda=10)
        z_init=sml.whiten.stbp2wn(u_init).flatten(order='F')
    except Exception as e:
        print(e)
        z_init=sml.whiten.sample()
    # h=1e-7; v=np.random.randn(sml.prior.L*sml.prior.J)
    # l,g=sml.get_geom(z_init,geom_ord=[0,1],whiten=True)[:2]; hess=sml.get_geom(z_init,geom_ord=[2],whiten=True)[2]
    # Hv=hess(v)
    # l1,g1=sml.get_geom(z_init+h*v,geom_ord=[0,1],whiten=True)[:2]
    ...
    # # center priors
    # sml.prior.mean = u_init
    # sml.whiten.mean = sml.whiten.stbp2wn(sml.prior.mean)
    ...
    winfMC=wht_geoinfMC(z_init,sml,args.step_sizes[args.alg_NO],args.step_nums[args.alg_NO],args.algs[args.alg_NO],transformation=sml.whiten.wn2stbp, MF_only=True, whitened=True, k=100)
    ...
    # filename='sml_'+args.algs[args.alg_NO]+'_dim'+str(len(u))+'_'+ctime
    ...
    try:
        ...
        med_f = np.median(samp,axis=0).reshape(np.append(sml.misfit.n_x,sml.misfit.n_t),order='F')
        mean_f = np.mean(samp,axis=0).reshape(np.append(sml.misfit.n_x,sml.misfit.n_t),order='F')
        std_f = np.std(samp,axis=0).reshape(np.append(sml.misfit.n_x,sml.misfit.n_t),order='F')
    except Exception as e:
        print(e)
        mean_f=0; std_f=0
        n_samp=samp.shape[0]
        for i in range(n_samp):
            samp_i=sml.prior.vec2fun(samp[i]) if sml.prior.space=='vec' else samp[i]
            mean_f+=samp_i/n_samp
            std_f+=samp_i**2/n_samp
        std_f=np.sqrt(std_f-mean_f**2)
        mean_f=mean_f.reshape(np.append(sml.misfit.n_x,sml.misfit.n_t),order='F')
        std_f=std_f.reshape(np.append(sml.misfit.n_x,sml.misfit.n_t),order='F')
#    if med_f is not None:
#        sml.misfit.plot_reconstruction(rcstr_imgs=med_f, save_imgs=True, save_path='./reconstruction/'+args.algs[args.alg_NO]+'_median')
#    sml.misfit.plot_reconstruction(rcstr_imgs=mean_f, save_imgs=True, save_path='./reconstruction/'+args.algs[args.alg_NO]+'_mean')
#    sml.misfit.plot_reconstruction(rcstr_imgs=std_f, save_imgs=True, save_path='./reconstruction/'+args.algs[args.alg_NO]+'_std')
