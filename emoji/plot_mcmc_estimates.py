...
# algorithms
algs=('winfmMALA',)#('wpCN','winfMALA','winfHMC','winfmMALA','winfmHMC','ESS')
alg_names=('w$\infty$-mMALA',)#('wpCN','w$\infty$-MALA','w$\infty$-HMC','w$\infty$-mMALA','w$\infty$-mHMC','ESS')
num_algs=len(algs)
pri_mdls=('q1p1','q2p2')
mdl_names=['STBP','STGP']
                ...
                if algs[i]+'_' in f_i and '_'+pri_mdls[j]+'_' in f_i:
                    try:
                        ...
                        try:
                            ...
                            med_f[i*num_mdls+j]=np.rot90(np.median(samp,axis=0).reshape(np.append(emj.misfit.sz_x,emj.misfit.sz_t),order='F'),k=3,axes=(0,1))
                            mean_f[i*num_mdls+j]=np.rot90(np.mean(samp,axis=0).reshape(np.append(emj.misfit.sz_x,emj.misfit.sz_t),order='F'),k=3,axes=(0,1))
                            std_f[i*num_mdls+j]=np.rot90(np.std(samp,axis=0).reshape(np.append(emj.misfit.sz_x,emj.misfit.sz_t),order='F'),k=3,axes=(0,1))
                            ...
                            mean_f[i*num_mdls+j]=np.rot90(mean_f[i*num_mdls+j].reshape(np.append(emj.misfit.sz_x,emj.misfit.sz_t),order='F'),k=3,axes=(0,1))
                            std_f[i*num_mdls+j]=np.rot90(std_f[i*num_mdls+j].reshape(np.append(emj.misfit.sz_x,emj.misfit.sz_t),order='F'),k=3,axes=(0,1))
                            # med_f[i]=None
...
for i in range(num_algs):
    for j in range(num_mdls):
        if len(med_f[i*num_mdls+j])!=0:
            emj.misfit.plot_reconstruction(rcstr_imgs=med_f[i*num_mdls+j], save_imgs=True, save_path=folder+'/reconstruction/'+algs[i]+'_median_'+pri_mdls[j], time_label=False)
        if len(mean_f[i*num_mdls+j])!=0:
            emj.misfit.plot_reconstruction(rcstr_imgs=mean_f[i*num_mdls+j], save_imgs=True, save_path=folder+'/reconstruction/'+algs[i]+'_mean_'+pri_mdls[j], time_label=False)
        if len(std_f[i*num_mdls+j])!=0:
            emj.misfit.plot_reconstruction(rcstr_imgs=std_f[i*num_mdls+j], save_imgs=True, save_path=folder+'/reconstruction/'+algs[i]+'_std_'+pri_mdls[j], time_label=False)