"""
Main function to obtain maximum a posterior (MAP) for the dynamic non-linear example Navier-Stokes equation (NSE)
----------------------
Shiwei Lan @ ASU, 2022
"""
...
def main(seed=2022):
    ...
    spat_args={'basis_opt':args.bass[args.bas_NO],'l':1,'s':2.0,'q':args.q,'L':2000}
    ...
    truth = nse.misfit.truth.cpu().numpy()
    ...
    if not hasattr(nse,'init_parameter'): nse._init_param(init_opt='prior_sample')
    param0 = nse.init_parameter
    if nse.prior.space=='fun': param0=nse.prior.vec2fun(param0)
    if args.whiten: param0 = nse.whiten.stbp2wn(param0).flatten(order='F')
    fun = lambda parameter: nse._get_misfit(nse.whiten.wn2stbp(parameter) if args.whiten else parameter, MF_only=False)
    def grad(parameter):
        param = nse.whiten.wn2stbp(parameter) if args.whiten else parameter
        g = nse._get_grad(param, MF_only=False)
        if args.whiten: g = nse.whiten.wn2stbp(parameter, 1)(g, adj=True)
        return g.squeeze()
    def hessp(parameter,v):
        param = nse.whiten.wn2stbp(parameter) if args.whiten else parameter
        Hv = nse._get_HessApply(param, MF_only=False)(nse.whiten.wn2stbp(parameter,1)(v) if args.whiten else v)
        if args.whiten:
        ...
        return Hv.squeeze()
    h=1e-6; v=nse.whiten.sample() if args.whiten else nse.prior.sample()
    # if args.whiten: v=nse.whiten.stbp2wn(v).flatten(order='F')
    ...
    # store the results
    map_v=nse.whiten.wn2stbp(res.x) if args.whiten else res.x
    map_f=nse.prior.vec2fun(map_v) if nse.prior.space=='vec' else map_v; funs=np.stack(FUN); errs=[] if len(ERR)==0 else np.stack(ERR)
    ...
    map_f=map_f.reshape(np.append(nse.misfit.sz_x,nse.misfit.sz_t),order='F')
    # name file
    ...
    savepath=os.path.join(os.getcwd(),'./invsol/MAP_'+f_name)
    if not os.path.exists(savepath): os.makedirs(savepath)
    # save
    filename='nse_MAP_dim'+str(len(map_f))+'_'+f_name+'_'+ctime+'.pckl'
    f=open(os.path.join(savepath,filename),'wb')
    pickle.dump([truth, spat_args, temp_args, map_f, funs, errs],f)
    f.close()
    # plot
    nse.misfit.plot_data(dat_imgs=map_f, save_imgs=True, save_path=savepath)
