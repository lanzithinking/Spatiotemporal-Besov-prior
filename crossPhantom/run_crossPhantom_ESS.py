"""
Main function to run elliptic slice sampling for the dynamic linear inverse problem of crossPhantom.
----------------------
Shiwei Lan @ ASU, 2022
"""
...
def main(seed=2022):
    ...
    store_eig = True
    xph = crossPhantom(**data_args, spat_args=spat_args, temp_args=temp_args, store_eig=store_eig, seed=seed, init_param=True)
    logLik = lambda u: -xph._get_misfit(u, MF_only=True)
    rnd_pri = xph.prior.sample
    ...
    try:
        ...
        u=xph.init_parameter if hasattr(xph,'init_parameter') else xph._init_param()
    ...
    filename='xph_ESS_dim'+str(len(u))+'_'+ctime
